...
- **在线预览（Vercel）**：部署后把你的地址填到这里
- **GitHub 仓库**：把你的仓库地址填到这里

> 推荐一键部署：
>
> [![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=你的仓库URL)
...
| 能力 | 选型 | 作用 |
| --- | --- | --- |
| 框架 | **React 19** + **Vite** | 项目脚手架 / HMR / 构建 |
| UI 组件库 | **Ant Design 6** | 按钮 / 表单 / 弹窗 / 消息提示 |
| 工作流画布 | **@xyflow/react**（原 React Flow，v12）| 节点拖拽、连线、缩放、平移、MiniMap、Controls |
| 全局状态管理 | **Zustand 5** | 统一管理 `nodes / edges / selectedNodeId / isRunning` 及所有操作（增删改查、撤销重做、运行） |
...
- ✅ 任何操作（新增/修改/删除/撤销重做/导入导出/运行）**全部走 store**，组件内零零散 `useState`
...
**双重防护**：① `isValidConnection` 在拖拽连线时实时校验，鼠标拖到会成环的目标上直接禁止松手；② `onConnect` 再次校验 + 弹窗。
