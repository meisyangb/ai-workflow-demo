...
│   ├── domains/
│   │   └── workflow.ts           # 领域基础（枚举/数据模型/DAG 算法/默认配置）
│   ├── schemas/
...
│   │   ├── authProvider.ts       # 鉴权：Token 持久化 + JWT exp 解码 + 过期清理 + login/refresh/logout 占位 + HTTP/WS 注入
...
│   │       ├── authProvider.test.ts        # Vitest 单元测试（17 例，InMemoryStorage + 手动时间源）
